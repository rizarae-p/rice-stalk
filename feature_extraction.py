...
for i in range(len(names)):
	...
	for j in glob.glob(mask_set_dir+"/*"):
		...
		features["h_cm"].append(aligned.shape[0]/95)
		features["w_cm"].append(aligned.shape[1]/95)
		features["area"].append(get_area(roi_colored))
		features["mode"].append(mode)
		features["gmean"].append(gmean)
		features["std"].append(std)

		# filewrite+=(",".join([j,folder,str(ratio),str(aligned.shape[0]),str(aligned.shape[1]),str(aligned.shape[0]/95),str(aligned.shape[1]/95),str(get_area(roi_colored)),mode,gmean,std])+"\n")
