...
h_main,w_main =0,0
csv = "stats.csv"
# features = {"img":[],"folder":[],"ratio":[],"h":[],"w":[],"h_cm":[],"w_cm":[],"area":[],"mode":[],"gmean":[],"std":[]}
# features_X = {"ratio":[],"h":[],"w":[],"h_cm":[],"w_cm":[],"area":[],"mode":[],"gmean":[],"std":[]}
features = {"img":[],"folder":[],"ratio":[],"h":[],"w":[],"area":[]}
features_X = {"ratio":[],"h":[],"w":[],"area":[]}
...
for i in range(len(names)):
	...
	img_shape = img.shape
	h_main,w_main = img_shape[0],img_shape[1]
	...
	for j in glob.glob(mask_set_dir+"/*"):
		...
		features["img"].append(j)
		features["folder"].append(folder)
		features["ratio"].append(ratio)
		features["h"].append(aligned.shape[0])
		features["w"].append(aligned.shape[1])
		# features["h_cm"].append(aligned.shape[0]/125)
		# features["w_cm"].append(aligned.shape[1]/125)
		features["area"].append(get_area(roi_colored))
		# features["mode"].append(mode)
		# features["gmean"].append(gmean)
		# features["std"].append(std)
		features_X["ratio"].append(ratio)
		features_X["h"].append(aligned.shape[0])
		features_X["w"].append(aligned.shape[1])
		# features_X["h_cm"].append(aligned.shape[0]/125)
		# features_X["w_cm"].append(aligned.shape[1]/125)
		features_X["area"].append(get_area(roi_colored))
		# features_X["mode"].append(mode)
		# features_X["gmean"].append(gmean)
		# features_X["std"].append(std)

X = pd.DataFrame(data=features_X)
scaler = StandardScaler()
X = scaler.fit_transform(X)
n_components = 2
# models = [GaussianMixture(n, covariance_type='full', random_state=0).fit(X) for n in n_components]
gmm = GaussianMixture(n_components=n_components,covariance_type='spherical',max_iter=1000,init_params='random',tol=0.000001)
gmm.fit(X)
labels = gmm.predict(X)
plt.scatter(X[:, 0], X[:, 1], c=labels, cmap='viridis')
plt.show()

features["class"] = list(labels)
dat = pd.DataFrame(data=features)
...
for index, row in dat.iterrows():
	...
	if class_label == 0:
		...
		class2+=1
	curr_mask = cv2.imread(row['img'])
	curr_mask_gray = cv2.imread(row['img'],0)
	if (len(prev_folder) > 0 and prev_folder != row['folder']) or index == 253:
		cv2.imwrite(prev_folder+".jpg",main)
		main = np.zeros(curr_mask.shape)
		print(class1,class2)
		class1 = 0
		class2 = 0
	overlay = np.full(curr_mask.shape,colors[class_label],dtype='uint8')
	ret,curr_mask_gray = cv2.threshold(curr_mask_gray,100,255,cv2.THRESH_BINARY)
	res_new = cv2.bitwise_and(overlay,overlay, mask= curr_mask_gray)
	main += res_new
